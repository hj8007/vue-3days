...
export default {
  ...
  methods: {
    ...
    async doSearch(bUpdated) {
      let currentPage = 1
      if (bUpdated) {
        currentPage = this.$refs.dataGrid.currentPage
      }
      ...
      setTimeout(() => {
        this.$refs.dataGrid.currentPage = currentPage
        this.$refs.dataGrid.changePage(currentPage)
        this.$refs.dataGrid.checkedItem = this.customer.id
      }, 500)
    }
  }
}
